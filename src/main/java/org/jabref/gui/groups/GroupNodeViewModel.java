...
public class GroupNodeViewModel {
            ...
            case KeywordGroup _ ->
                // KeywordGroup is parent of LastNameGroup, RegexKeywordGroup and WordKeywordGroup
                    groupNode.getParent()
                            ...
                            .map(groupParent -> !(groupParent instanceof AutomaticKeywordGroup || groupParent instanceof AutomaticPersonsGroup))
                            .orElse(false);
            case SearchGroup _, AutomaticKeywordGroup _, AutomaticPersonsGroup _, TexGroup _ -> true;
            case null -> throw new IllegalArgumentException("Group must not be null");
            default ->
    ...
    public boolean canAddGroupsIn() {
        AbstractGroup group = groupNode.getGroup();
        return switch (group) {
            case AllEntriesGroup _, ExplicitGroup _ -> true;
            case KeywordGroup _ ->
                // KeywordGroup is parent of LastNameGroup, RegexKeywordGroup and WordKeywordGroup
                    groupNode.getParent()
                            .map(GroupTreeNode::getGroup)
    ...
    public boolean canRemove() {
        ...
        return switch (group) {
            ...
            case SearchGroup _, AutomaticKeywordGroup _, AutomaticPersonsGroup _, TexGroup _ -> true;
            case null -> throw new IllegalArgumentException("Group must not be null");
            default -> throw new UnsupportedOperationException("canRemove method not yet implemented in group: "
                    + group.getClass().getName());
        };
    }
    ...
    public boolean isEditable() {
        ...
        return switch (group) {
            ...
            case SearchGroup _, AutomaticKeywordGroup _, AutomaticPersonsGroup _, TexGroup _ -> true;
            case null -> throw new IllegalArgumentException("Group must not be null");
            default -> throw new UnsupportedOperationException("isEditable method not yet implemented in group: "
                    + group.getClass().getName());
        };
    }
